/*
A powerful 64x64 spider can spawn in any burrow in maints.
...
Essentially the spider counterpart for the kaiser
*/

/datum/storyevent/terror
	id = "terror"
	name = "terror"

	weight = 1

	event_type = /datum/event/terror
	event_pools = list(
...
/datum/event/terror/can_trigger()
	if(!GLOB.all_burrows.len)
		log_and_message_admins("Terror spawn failed: no burrows detected.")
		return FALSE
	return TRUE

/datum/event/terror/setup()
...
/datum/event/terror/start()
	spawn_mobs()

/datum/event/terror/proc/spawn_mobs()
	new /mob/living/carbon/superior_animal/giant_spider/tarantula/emperor/reaper_spider(enter_burrow)
	for(var/R in terror_rutinue)
		new R(enter_burrow)
	...
	log_and_message_admins("Sending Terror to [jumplink(exit_burrow)]")
