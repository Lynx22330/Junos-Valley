...
/datum/sex_controller/proc/update_erect_state()
	var/obj/item/organ/penis/penis = user.organ_list_by_process(OP_PENIS)
	if(penis)
...
/datum/sex_controller/proc/damage_from_pain(pain_amt)
	...
	var/obj/item/bodypart/part = user.get_bodypart(BP_CHEST)
	if(!part)
		return
...
/datum/sex_controller/proc/can_ejaculate()
	if(!user.organ_list_by_process(OP_TESTICLES) && !user.organ_list_by_process(OP_VAGINA))
		return FALSE
