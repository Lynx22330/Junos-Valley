...
/datum/sex_action/suck_nipples/shows_on_menu(mob/living/carbon/human/user, mob/living/carbon/human/target)
	if(user == target)
		return FALSE
	if(!target.organ_list_by_process(OP_BREASTS))
		return FALSE
	return TRUE
...
/datum/sex_action/suck_nipples/can_perform(mob/living/user, mob/living/target)
	if(user == target)
		return FALSE
	if(!get_location_accessible(target, BP_CHEST))
		return FALSE
	if(!get_location_accessible(user, BP_MOUTH))
		return FALSE
	if(!target.organ_list_by_process(OP_BREASTS))
		return FALSE
	return TRUE
...
/datum/sex_action/suck_nipples/on_perform(mob/living/carbon/human/user, mob/living/carbon/human/target)
	...
	var/milk_to_add = min(max(target.organ_list_by_process(OP_BREASTS).breast_size, 1), target.organ_list_by_process(OP_BREASTS).milk_stored)
	if(target.organ_list_by_process(OP_BREASTS).lactating && milk_to_add > 0 && prob(25))
		user.reagents.add_reagent(/datum/reagent/consumable/breastmilk, milk_to_add)
		target.organ_list_by_process(OP_BREASTS).milk_stored -= milk_to_add
